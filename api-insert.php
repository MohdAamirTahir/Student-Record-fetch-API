...
$data = json_decode(file_get_contents("php://input",true)); // data read karne k liye function use karte h or is formate k through hum koisa bhi data read kar sakte h and row data ko read karne k liye hum php://input ka use karte h
    $student_id = $data->student_name; // error de raha tha $data['sid'] per
    $age = $data->age;
    $city = $data->city;
    include("config.php");
    $sql = "INSERT INTO students (student_name, age, city) VALUES ('$student_id', $age, '$city')";
    if(mysqli_query($conn, $sql)){
        echo json_encode(array('message' => 'Record is inserted.', 'status' => true));
    } else {
